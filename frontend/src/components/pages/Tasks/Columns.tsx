...
const Columns = ({ tasks }: TColumsProps) => {
    ...
    const onSortEnd = async (event: DragEndEvent) => {
        ...
        if (active.id !== over?.id) {
            setTasksState((items) => {
                const oldIndex = items.findIndex((item) => item.id === parseInt(active.id.toString()));
                const newIndex = items.findIndex((item) => item.id === parseInt(over?.id.toString() ?? ""));
    ...
    const onDragEnd = async (event: DragEndEvent) => {
        ...
        if (
            !Object.keys(ETaskStatus).includes(overId.toString())
        ) {
                    ...
                    (key) => {
                        return (
                            <Droppable key={key} id={key}>
                                    ...
                                    <SortableContext items={tasksState.filter((task) => task.status === ETaskStatus[key])} strategy={verticalListSortingStrategy}>
                                            {tasksState
